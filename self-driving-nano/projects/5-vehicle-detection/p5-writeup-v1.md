...
I then checked to make sure the distribution of training and testing were balanced.
...
Here we implement a sliding-window technique and use a trained classifier to search for vehicles in images.

#### 3.1 &nbsp; Deciding what scales to search and how much to overlap windows

In sliding window search, we take the features we extracted using HOG (for a single image or video frame), then we subsample it using multiple windows which we slide over the sections of the image containing the road.

For each part of the image a window passes over, the classifier makes a prediction of whether that patch of the image contains a vehicle. The resulting output is a list of rectangle objects corresponding to the windows that generated a positive "car" prediction.

To prototype this, I repurposed the [`draw_boxes()`]() function that used in the project preparation exercises. Below is an example of the output using a single window size.

<img src='results/window-one-size.png' width="60%"/>

Once I got the initial version working, I then explored combinations of windows with different sizes and positions. The general strategy was to blanket the road area with overlapping windows of different sizes with a priority of detecting vehicles in close proximity.

Here is a configuration with showing a uniform overlap and window size. As you can imagine, this wouldn't be optimal for detecting vehicles of different sizes moving at various distances and angles.

<img src='results/window-grid.png' width="60%"/>

However, too many window variations can create a lot of detection redundancies. For example, one of my methods yielded this output. As you can see, this produces more detections than necessary and results in a detection area that's much larger than the car. We want our detection areas to be a little.
...
* Using small windows near the top part of the road increased true positives (since cars appearing in this part of the image are farther away, and therefore appear smaller)
...
With the output from the sliding windows method, we can then record the positions of positive detections. From there we construct a heatmap with individual blobs in the heatmap corresponding to a different vehicle.

Sliding window detections:

<img src='results/detection.png' width="60%"/>

Resulting heatmap using the [`add_heat()`](https://github.com/tommytracey/udacity/blob/master/self-driving-nano/projects/5-vehicle-detection/p5-vehicle-detection-final.py#L63) function:
...
I then used `scipy.ndimage.measurements.label()` to generate labels that surround each blob on the heatmap.
