...
# 
# 
# ## Project 3: Behavioral Cloning ( Track 2 )
# August 2017
# 
...
# ### Track 2 Data Set (self-generated)

# #### Generic driving data
...
# Load data into list
with open('data/track2/trouble/driving_log.csv', newline='') as f:
    track_data = list(csv.reader(f, skipinitialspace=True, delimiter=',', quoting=csv.QUOTE_NONE))

# Load data from .csv and preview it in Pandas dataframe
track_df = pd.read_csv('data/track2/trouble/driving_log.csv', header=0)
...
index = random.randint(0, len(track_data))
img_dir = 'data/track2/IMG/'

center_img_orig = mpimg.imread(img_dir + track_data[index][0].split('/')[-1])
left_img_orig = mpimg.imread(img_dir + track_data[index][1].split('/')[-1])
right_img_orig = mpimg.imread(img_dir + track_data[index][2].split('/')[-1])

center_steer = track_data[index][3]
...
# Generic driving data distribution
...
sources = ['track1', 'track2', 'track2/trouble']
...
# ang_corr = 0.35   # fixed steering angle correction for left and right cameras
...
# 
# **NOTE:**
# The crop function is not actually performed during pre-processing. It happens via the generator. But, for visualization purposes in this notebook, we'll start showing images in their cropped formed now...so that we can see how other transformations we're making will affect what the model "sees" later in the pipeline.
...
# ### 3.3 | Flip the images
# The flip function is also not performed during pre-processing. It happens via the generator. But, for visualization purposes in this notebook, we'll start showing images in their flipped form now...so that we can see how other transformations we're making will affect what the model "sees" later in the pipeline. It also gives a better view of the steering angle distribution. 
...
# If needed, we can remove a portion of the zero angle data to flatten the distribution so that our model receives a more balanced set of training samples for each bin of steering angles. This will ensure that the model isn't better at steering in one direction versus another (or not steering at all!).
...
keep = 10.0   # set very high for Track 2 because the distribution is fairly balanced I didn't want to lose data via flattening
...
keep = 15.0       # the keep factor for records during final flattening (X times above the mean)
source = default_source
...
## Create and reset the model  ** V1 - NVIDIA ** LOADED WEIGHTS FROM TRACK 1 **
...
model.load_weights('models/checkpoints/model_v47b_e03_0.020.h5')
...
## Train and save the model ** track2 generic **
