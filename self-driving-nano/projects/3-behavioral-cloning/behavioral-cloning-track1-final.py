...
# 
# 
# ## Project 3: Behavioral Cloning ( Track 1 )
# August 2017
# 
...
# ### New Data Set (self-generated data)
# This is data recorded using the driving simulator. 
...
# - all of the images are consistently bright; no glare, no darkness, and no shadows that you'd usually encounter with normal driving (NOTE: I used the fastest and lowest resolution images for training. The higher resolution versions do include things like shadows.)
# 
...
# 
# That said, many of the other items above can create biases in our model and cause it to overfit the specific driving conditions within this particular simulation. If we wanted to train our model to drive on other tracks with other conditions, we'd need to correct for these tendencies so that our model learns to drive in a variety of conditions. We'd do this by augmenting our training data to mimic other driving conditions. However, for the scope of this project we'll just focus on getting the car to succcessfully navigate this particular track. Then, we'll try to evolve the model by training it with Track 2 data in hopes of creating a single model that can successfully navigate both tracks.
# 
# But first let's look at our target data (steering angles) to see if there's anything else we need to correct for...
...
# 
# It also seems there may be an imbalance in left vs. right turning data (although not as much as I had expected). It's not clear how big of an impact this would have on the model, but there's a chance the model could learn to turn more effectively in one direction than the other. Just to be safe we'll correct for this by creating flipped copies of our training images to ensure there are equal quantities of left and right turns. 
...
# 
# **Data Sources**: 
# - There are two tracks. All of the training and validation data in this notebook is from **Track 1**.
# - Udacity provides an intial set of "good" data to get us started.
# - Additional "self-generated" data is gathered by running the simulator myself. 
# 
# **Training Data**:
# - The training data set includes the original image data captured from the simulator on Track 1, plus any additional data generated via augmentation. 
# 
# **Validation Data**:
# - The validation data will contain the original image data captured from the simulator on Track 1, with only some minor pre-processing to make the data clean and usuable for testing. No other pre-processing or augmentation is done on the validation set. This ensures we can properly validate whether changes we're making to the model and to the training data are increasing or decreasing the model's performance. 
# 
# **Test Data**:
# - We'll test the model's ability to generalize by running it in the simulator. So, the simulator input images will serve as our test data. Some cropping and resizing are done to these images to match the training data inputs for the model. 

# In[11]:


## Define data sources; used for pulling images from the correct directory

# Available data sources/groupings
sources = ['track1', 'track1/trouble-spots', 'track1/recovery', 'track1/turns', 'track1/udacity']

# Specific data source used for building the pipeline below
...
# 3. Cast all of the steering data as floats. By default they are recorded in the .csv as strings. 
...
# #### Recombine Training Data + Quick Sanity Check
...
# ### 3.1 | Seperate C/L/R camera data and adjust steering angles for left and right turns
...
# 
# NOTE: Initially, I used a fixed steering correction value. But in the end, I settled on a correct factor of 2.0 &mdash; i.e., the center steering angle is multiplied by 2.0 for left and right turns.

# In[28]:


turn_thresh = 0.15   # angle threshold used to identify left and right turns
# ang_corr = 0.27      # fixed steering angle correction for left and right cameras
ang_fact = 2.0       # multiplication factor for steering angle correction 
...
#     new_angle = min((abs(angle)+ang_corr), 1.0)
    new_angle = min((abs(angle)*ang_fact), 1.0)
...
# Unfortunately, I went through several iterations not realizing that right turns were not included in my training data (face palm). So I added this sanity check and expanded it to include a more detailed look at the distribution of turns for each camera angle. While I was adding more data to the training set and experimenting with my stearing angle correction function, this turned out to be a really useful tool because it allowed me to easily spot problems (i.e., stearing angles that were over or under represented). 
...
# ### 3.2 | Crop the images (preview only)
...
# 
# **NOTE:**
# The crop function is not actually performed during pre-processing. Instead, it is done via the generator (which we'll cover further along in this notebook). But we show the cropping function at this point for visualization purposes. That is, we want to visualize how the transformations we're making will affect what the model ultimately "sees" later in the pipeline.
...
# Number of pixels to remove from (top, bottom), (left, right)
crop_set = (70, 20), (0, 0)   
    
# Original input_shape (160, 320, 3) 
...
# ### 3.3 | Flip the images
# This function is also executed later in the pipeline within the generator. But, again, I decided to start showing it during the pre-processing stage since it inherently adds more images to the training set, while balancing the steering angle distribution. These are important factors to understand before applying further transformations. And, doing this earlier in the process helps prevent some data loss when we attempt to equalize the distribution in the final step of pre-processing. The equalization step (randomly) removes a significant chunk of data, so it's better to add the flipped images beforehand as it increases the chances that at least one version of an image is included in the training data (i.e., it adds more samples and greater variety to our training set).
...
# Again, we also crop the images here for demonstration purposes, so we get a better idea of what the model will ultimately see.
...
# We can see below that the nuber of left and right turns in each bin is now perfectly balanced. And, we now have twice as many training images!
...
def equal_dist(data, keep):
    ...
    - Otherwise the keep prob for that bin is set to bring the number of samples for that bin down toward
    the average, depending on how aggresively one sets the keep factor. For example, a 'keep' factor of 2
    will keep all samples below 2x the distribution average.
...
# #### New Distribution
# You can see that the distribution is now flattened a bit, so there is a higher percentage of turning data. But this means we're now removing a significant portion of our training data. We'll compensate for this in the augmentation phase.
...
# Make the original image shape has been maintained (i.e. no cropping or resizing yet in the pipeline) and 
# that we're able to correctly pull the image arrays from their directory
...
# #### Sanity Check
# These metrics are used to verify that the size and shape of the data set make sense...and that we didn't break anything during pre-processing
...
# Number of training targets (steering angles)
len(y_train_self)
...
# Make sure training features are image filenames
X_train_self[0]
...
# Make sure the training set includes 'right' camera angles (this was broken at one point)
...
# Preview a training target (steering angle)
y_train_self[0]
...
# ### 3.6 | Pipeline status check &mdash; Where are we at?
...
# 
# **4** - Feed the data into a generator to undergoe further augmentation. Training data undergoes all transformations (e.g., crop, flip, resize, smoothing, random brightness, affine transformations). Validation data only undergoes crop, flip, and resizing.  <br>
# **5** - Feed the augmented data into the model where it is first normalized <br>
...
# #### Closer Look at Smoothing Effect
# As you can see, it's barely detectible. After some experimentation, I found that oversmoothing negatively effected model peformance, so I scaled it back. 
...
orig_img = mpimg.imread(get_img_dir(source) + train_data[index][0])
smooth_img = smooth(train_data[index][0])
...
'''Note: I experimented with this, but decided to remove it from the final model because the Track 2 data provided
sufficient data for horizon shifts. That said, this is a useful tool for simulating gradient changes on the track 
(i.e. driving up hill or down hill).'''
...
# ### 4.4 | Affine Transformations (rotation, shift, zoom)
...
# ### 4.6 | Integrate all transformations into single function
...
def generator(images, angles, val=False, display=False):
    ...
    img : one resized and cropped version of original image
    img_flip : one flipped version of img
    img_aug (2 images) : augmented versions of the first two images (for training only)
    
    Arguments:
    images: a list of image filenames
    angles: a list of angles
    source: the original data source (e.g. track1 vs track2)
    val: whether the data is being generated for validation (False by default)
    display: whether the data is being generated for display purposes within the notebook
...
img_ratio = 2    # number of augmented images to create for each image in sample
...
batch_size = img_ratio * len(y_batch) * 2
...
for i in range(len(sample)):
    img = mpimg.imread(get_img_dir(source) + sample[i-2][0])
...
## Map for images below
'''
Column 1: cropped and resized version of original image
Column 2: flipped version
Column 3: augmented version of column 1 image
Column 4: augmented version of column 2 image
''' 
...
## Load a previous version of a model if needed (for experimentation purposes)
...
## Train and save the model ** generic driving data **
...
# In this section, I make an attempt to visualize what the model "sees" by mapping a set of model filters back onto a sample of training images. The purpose of this exercise is to idenity strengths and weaknesses of the model, and in turn, inform further refinements to the pipeline.  
# 
# The functions below were borrowed from fellow Udacity student [Li Ma](https://github.com/dolaameng/Udacity-SDC_Behavior-Cloning/blob/master/understand_model.ipynb) with only minor modifications.
...
def visualize_outputs(img_files, layer="block3_conv2", topn=8, img_size=(64, 128)):
    ...
    # load the model
    model = load_model('visuals/t1-model-good.h5')        
...
# #### Visualization of *straight* driving examples
# We can see from the images below which aspects of the road the model focuses on when the car is driving straight. In general, it seems that the model identifies the most useful features. 

# In[18]:


## Visualize sample: Straight driving, Center camera

no_turn_imgs = []

for row in track_clean:
    if 0.1 > row[3] > -0.1:
        no_turn_imgs.append(get_img_dir(default_source) + row[0])
    
no_turn_sample = random.sample(no_turn_imgs, 8)

visualize(no_turn_sample)


# #### Visualization of right and left turns
# We can see from the images below which aspects of the road the model focuses on when the car is turning. In many cases, the model identifies the most useful features. But we also see many examples where the model's focus seems ambigious...or it's focusing on less important aspects of the road.
...
## Visualize sample: Left turns, Center camera

left_turn_imgs = []

for row in track_clean:
    if row[3] < -0.35:
        left_turn_imgs.append(get_img_dir(default_source) + row[0])
    
left_turn_sample = random.sample(left_turn_imgs, 8)

visualize(left_turn_sample)
